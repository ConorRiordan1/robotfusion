...
class Camera_subscriber(Node):
    ...
    def __init__(self):
        ...
        super().__init__('object_found_publisher')
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for c in det[:, -1].unique():
                    objects=[]
                    #print(self.names[int(c)],) #This line prints what is seen on the camera
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    object_found=[str(xyxy),str(self.names[int(c)])]#prints coordinates of picture along with the prediciton name. First coordinate is top left of the box. Second coordinate is bottom right of box. X coordinates span from (0-640)
                    #print(object_found)
                    encoded_objects=json.dumps(object_found)
                    msg=String()
                    msg.data = encoded_objects
...
class objectpublisher(Node):
    def __init__(self,n):
        super().__init__('object_found_publisher')
        if n==1:
            self.publisher_= self.create_publisher(String,'/object_found',10)
        timer_period = 0.5
